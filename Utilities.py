...
def iterGuidance(oglist, og, PathtoOutput, guidanceIter, seqcutoff, colcutoff, rescutoff, mode):
	...
	if 'OG' in og:
		...
		if str(mode) != "ng":
			print "\n" + og + ": Running Guidance"
			print 'bash exeGuidance_2.2.sh -i ' + path2og + ' -o ' + tempdir + ' -t 3 -c ' + outdir + ' -g ' + str(guidanceIter) + ' -s ' + str(seqcutoff) + ' -l ' + str(colcutoff) + ' -r ' + str(rescutoff) + ' -m ' + str(mode)
			os.system('bash exeGuidance_2.2.sh -i ' + path2og + ' -o ' + tempdir + ' -t 3 -c ' + outdir + ' -g ' + str(guidanceIter) + ' -s ' + str(seqcutoff) + ' -l ' + str(colcutoff) + ' -r ' + str(rescutoff) + ' -m ' + str(mode))
